...
let serve = () => {
    browserSync({
        ...
        reloadDelay: 250,
        server: {
            baseDir: ["temp", "src", "src/html"],
        },
    });

    watch("src/scripts/*.js", series(lintJS, transpileJSForDev)).on(
        "change",
        reload
    );

    watch("src/styles/**/*.scss", series(compileCSSForDev)).on(
        "change",
        reload
    );
...
async function listTasks() {
    ...
    exec("gulp --tasks", function (error, stdout, stderr) {
        if (null !== error) {
            process.stdout.write(
                'An error was likely generated when invoking the "exec" program in the default task.'
            );
        }

        if ("" !== stderr) {
            process.stdout.write(
                'Content has been written to the stderr stream when invoking the "exec" program in the default task.'
            );
        }
